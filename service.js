...
app.service('movieService', function(){
  ...
  var personalCollection = [];
    ...
    function addToCollect(movie){
      if (money.amount < 3){
        show = false;
        alert("You don't have enough budget to add more movies!")
      } else {
        console.log("hello")      
        personalCollection.push(allMovies[movie]);
  ...
  function removeMovie (movie){
    allMovies.push(personalCollection[movie]);
    personalCollection.splice(movie,1);

    if (budget < 3){
      show=false;}

    else if (!personalCollection[0]){
      show=true;
    }
  }
// });
...
// app.service ('personalCollectionService', function(){
  ...
  return {allMovies: allMovies, personalCollection:personalCollection, addToCollect:addToCollect,
           show: show, money: money, removeMovie: removeMovie};  
});
